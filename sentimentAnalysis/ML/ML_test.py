...
class sentimentAnalysis(object):
    ...
    def initialize(self):
        self.stop_words = uc.get_stop_words(self.stopwords_path)
    ...
    def predict_score(self, docs):
        ...
        uc.process_x(docs, self.vectorizer, self.tfidftransformer)
        # print(new_tfidf)

        data = pd.read_csv('../data/x.csv',
                           encoding='utf-8')
        predicted = self.clf.predict_proba(data)
    ...
    def analyze(self, text):
        print(text)
        # docs = [uc.chinese_word_cut(sentence) for sentence in text]
        docs = uc.so(text)
        # print(docs)
        # text_corpus = self.replace_text(text)
        result = self.predict_score(docs)
        ...
        # print('差： {} 一般：{} 好： {}'.format(neg, mid, pos))
        print('差： {} 好： {}'.format(neg, pos))
